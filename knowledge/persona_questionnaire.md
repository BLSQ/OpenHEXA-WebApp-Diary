...
>    (human or agent) builds the app — it records *who we are building for* and *what is in/out
>    of scope for v1*.
...
- Lines marked **⭐** are the cruxes: if these flip, the product changes shape. They have a
  **Notes** line — please add a sentence of *why*.
...
- **Primary user:** NMCP staff (National Malaria Control Programme).
...
1. Primary user is an **NMCP employee** (vs. external consultant/partner). ⭐
   - [ ] Yes  [ ] No  [ ] Open
   - **Notes:**
...
3. If a team: **all members** run pipelines (vs. one runs, others view).
   - [ ] Yes (all run)  [ ] No (one runs)  [ ] Open
4. The user holds an **M&E / surveillance** role specifically.
   - [ ] Yes  [ ] No  [ ] Open
...
7. **Excel** is the high-water mark we must assume.
   - [ ] Yes  [ ] No  [ ] Open
8. They are **regular DHIS2-suite** users (analytics, pivot, maps).
   - [ ] Yes  [ ] No  [ ] Open
9. We may assume they can read/edit a **JSON config file** unaided. *(expected: No.)*
   - [ ] Yes  [ ] No  [ ] Open
10. We may assume they can read a **log / stack trace** to understand a failure. *(expected: No.)*
    - [ ] Yes  [ ] No  [ ] Open
11. They are comfortable with the concept of **parameters/arguments**.
    - [ ] Yes  [ ] No  [ ] Open
12. They know what a **"pipeline" / DAG** is as a concept.
    - [ ] Yes  [ ] No  [ ] Open
13. **English** is sufficient for the actual NMCP users (not just BLSQ). *(expected: Yes for v1.)*
    - [ ] Yes  [ ] No  [ ] Open
...
15. User already knows **which pipeline to run in what order**. *(expected: No.)* ⭐
    - [ ] Yes  [ ] No  [ ] Open
    - **Notes:**
...
19. App must **recommend the next step** (vs. only display the map and let them choose). ⭐
    - [ ] Yes (recommend)  [ ] No (display only)  [ ] Open
    - **Notes:**
20. User will know which **alternative** to pick among mutually-exclusive pipelines.
    - [ ] Yes  [ ] No  [ ] Open
...
30. The user already has an **OpenHEXA account** with run permissions (granted by someone else).
    - [ ] Yes  [ ] No  [ ] Open
31. The user should **never install a missing pipeline** themselves. *(expected: No — ask BLSQ.)* ⭐
    - [ ] Yes  [ ] No  [ ] Open
    - **Notes:** *(see I.1 Q51 — note install is one-click, which complicates this)*

## F. Context & environment of use

32. **Desktop/laptop only** (no tablet/phone). *(spec assumes Yes.)*
    - [ ] Yes  [ ] No  [ ] Open
...
69. **Dependency locking** (grey-out until upstream succeeds) — explicitly **deferred** past v1. *(spec open-Q2.)*
    - [ ] Yes (deferred)  [ ] No (needed in v1)  [ ] Open
...
71. Acceptable that status reflects only the **current session's** runs (vs. full persistent history). ⭐
    - [ ] Yes (session-only OK)  [ ] No (must persist)  [ ] Open
    - **Notes:**
